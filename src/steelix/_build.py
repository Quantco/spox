...
class Builder:
    ...
    class ScopeTree:
        ...
        This structure is the base of the least-enclosing-scope algorithm. Every value (Arrow), and hence
    ...
    graphs: Set["Graph"]
    # Arguments, results
    arguments_of: Dict["Graph", List[Arrow]]
    results_of: Dict["Graph", List[Arrow]]
    source_of: Dict["Graph", Node]
    # Arguments found by traversal
    all_arguments_in: Dict["Graph", Set[Arrow]]
    claimed_arguments_in: Dict["Graph", Set[Arrow]]
    # Scopes
    ...
    def get_intro_results(
        request_results: Dict[str, Arrow], set_names: bool
    ) -> List[Arrow]:
        """
        ...
        """
        # Created arrows all have the same op
        arrows = list(intros(*request_results.values()))
        for key, arrow in zip(request_results, arrows):
            if set_names:
                arrow._rename(key)
        return arrows

    def discover(self, graph: "Graph") -> Tuple[Set[Arrow], Set[Arrow]]:
        """
    ...
    def compile_graph(
        ...
        """
        Compile a given Graph into a BuildResult. Handles naming of all the Arrows/Nodes and only adds Nodes to a
